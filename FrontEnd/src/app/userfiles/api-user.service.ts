...
export class ApiUserService {
  ...
  getActiveUsers(): Observable<User[]> {
    return this.http.get<User[]>(this.usersUrl + 'active')
      .pipe(catchError(this.handleError<User[]>(`getActiveUsers`, [])));
  }

  getTitles(): Observable<Title[]> {
    return this.http.get<Title[]>(this.titlesUrl)
      .pipe(catchError(this.handleError<Title[]>(`getTitles`, [])));
  }

  getTypes(): Observable<Type[]> {
    return this.http.get<Type[]>(this.typesUrl)
      .pipe(catchError(this.handleError<Type[]>(`getTypes`, [])));
  }

  getUser(id: Number): Observable<User> {
    return this.http.get<User>(this.usersUrl + id)
      .pipe(catchError(this.handleError<User>(`getUser with id=${id}`)));
  }

  updateUser(user: User): Observable<any> {
    return this.http.put((this.usersUrl + user.id), user)
      .pipe(catchError(this.handleError<any>(`updateUser with id=${user.id}`)));
  }
  ...
  deleteUser(user: User): Observable<any> {
    user.isActive = false;
    return this.http.put((this.usersUrl + user.id), user)
      .pipe(catchError(this.handleError<any>(`deleteUser with id=${user.id}`)));
  }
